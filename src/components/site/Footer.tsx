...
export function Footer({ brand, extraLinks = [] }: Props) {
  ...
  const onSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Subscribed", { description: "Welcome to the journal." });
      setEmail("");
    } catch (err) {
      toast.error("Couldn't subscribe", { description: err instanceof Error ? err.message : "Try again." });
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <footer style={{ background: "#1a1a1a", color: "#faf9f7" }}>
      <div className="mx-auto grid max-w-[1400px] grid-cols-1 gap-10 px-6 py-16 md:grid-cols-12 md:px-10">
        ...
        <nav className="flex flex-wrap items-start gap-x-8 gap-y-3 md:col-span-4">
          {links.map((l) => (
            <a
              key={l}
              href="#"
        ...
        <div className="md:col-span-4">
          <div className="text-[10px] uppercase tracking-[0.3em] text-[#b0a89a]">The Journal</div>
          <p className="mt-2 text-sm text-[#b0a89a]">
            Quarterly notes on materials, projects, and new arrivals.
          </p>
          <form onSubmit={onSubscribe} className="mt-4 flex border-b border-white/15">
            <input
              required
              type="email"
              value={email}
            ...
            <button
              disabled={busy}
              className="px-3 text-[11px] uppercase tracking-[0.3em] text-[#c8b89a] disabled:opacity-50"
            >
              {busy ? "…" : "Subscribe →"}
            </button>
          </form>
